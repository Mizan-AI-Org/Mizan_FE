...
export default function ShiftReviewsView() {
    const { t } = useLanguage();
    ...
    React.useEffect(() => {
        if (rawReviews) {
            const count = Array.isArray(rawReviews) ? rawReviews.length : 0;
    ...
    React.useEffect(() => {
        if (error) {
            logError({ feature: "shift-reviews-admin", action: "fetch-error" }, error);
        }
    }, [error]);
