        ...
        if (window.LuaPop && !initialized.current) {
            try {
                window.LuaPop.init({
                    ...
                    runtimeContext: `Restaurant: ${user.restaurant_data?.name || "Unknown"} (ID: ${user.restaurant_data?.id || user.restaurant}), User: ${user.first_name} ${user.last_name} (ID: ${user.id}), Role: ${user.role}`,
