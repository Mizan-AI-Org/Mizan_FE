...
import { MapPin, Navigation, Crosshair, Save, AlertCircle, Loader2 } from "lucide-react";
...
export default function GeolocationMapSettings({
  ...
  const [latInput, setLatInput] = useState<string>(() => fallbackCenter.lat.toString());
  const [lngInput, setLngInput] = useState<string>(() => fallbackCenter.lng.toString());
  const [radiusMeters, setRadiusMeters] = useState<number>(radius || 0);
  const [isGettingLocation, setIsGettingLocation] = useState(false);
  const [mapCenter, setMapCenter] = useState<L.LatLngExpression>(fallbackCenter);
  const [polygonPoints, setPolygonPoints] = useState<Array<[number, number]>>(geofencePolygon);
  ...
  const handleAddPolygonPoint = useCallback((pointLat: number, pointLng: number) => {
  ...
  const handleSave = async () => {
    ...
    if (Number.isNaN(parsedLat) || Number.isNaN(parsedLng) || Number.isNaN(radiusMeters)) {
  ...
  return (
    <Card className="shadow-soft">
      <CardHeader>
        ...
        <CardDescription>
          Set your restaurant's location and define the geolocked zone for staff clock-in compliance
        </CardDescription>
      </CardHeader>
      <CardContent className="space-y-6">
        <div className="flex flex-col gap-4 rounded-lg border border-border p-4 sm:flex-row sm:items-center sm:justify-between">
          <div className="space-y-1">
            ...
            <p className="text-xs text-muted-foreground">Require team members to be within your geolocked zone before clocking in.</p>
          </div>
          <Switch checked={geofenceEnabled} onCheckedChange={onToggleGeofence} className="sm:self-center" />
        </div>
        ...
        <Button onClick={getCurrentLocation} disabled={isGettingLocation} variant="outline" className="w-full">
          {isGettingLocation ? (
            <>
        ...
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <div className="flex w-full flex-col gap-2 sm:w-auto sm:flex-row sm:items-center">
            <Button variant={isDrawing ? "destructive" : "outline"} onClick={handleStartStopDrawing} className="w-full sm:w-auto">
              {isDrawing ? "Finish Geolock Zone" : "Define Geolock Zone"}
            </Button>
            <Button variant="outline" onClick={handleClearPolygon} disabled={!polygonPoints.length} className="w-full sm:w-auto">
              Clear Zone
            </Button>
          </div>
          <Badge variant={hasPolygon ? "default" : "secondary"} className="justify-center px-3 py-1 text-xs">
            {hasPolygon ? `${polygonPoints.length} points captured` : "Zone not defined"}
          </Badge>
        </div>
        ...
        <div className="relative w-full h-[320px] overflow-hidden rounded-lg border-2 border-border sm:h-[420px]">
          <MapContainer center={mapCenter} zoom={15} scrollWheelZoom className="h-full w-full">
            ...
            <Marker position={[lat, lng]} draggable icon={markerIcon} eventHandlers={{ dragend: handleMarkerDrag }} />
            {radiusMeters > 0 && (
              <Circle center={[lat, lng]} radius={radiusMeters} pathOptions={{ color: "#2563eb", fillColor: "#3b82f6", fillOpacity: 0.15 }} />
            )}
            {hasPolygon && (
              <Polygon positions={polygonPoints.map(([pointLat, pointLng]) => [pointLat, pointLng])} pathOptions={{ color: "#16a34a", fillColor: "#22c55e", fillOpacity: 0.2 }} />
            )}
          </MapContainer>
        </div>
        ...
        <div className="flex flex-col gap-3 rounded-lg border border-accent/20 bg-accent/10 p-4 sm:flex-row sm:items-start">
          ...
          <div className="flex-1 space-y-1">
            <p className="text-sm font-medium">{geofenceEnabled ? "Geofencing Active" : "Geofencing Disabled"}</p>
            <p className="text-xs text-muted-foreground">
              Staff must clock in within {radiusInKm} km of the map marker. Add a geolock zone to further restrict clocking to the polygon boundary.
            </p>
          </div>
        </div>
        ...
        <div className="space-y-4">
          ...
          <Slider id="radius-slider" min={50} max={5000} step={50} value={[radiusMeters]} onValueChange={(value) => setRadiusMeters(value[0])} className="w-full" />
        ...
        <div className="space-y-2">
          ...
          <Input id="radius-input" type="number" value={radiusMeters} onChange={(e) => setRadiusMeters(parseInt(e.target.value) || 0)} placeholder="Enter radius in meters" />
        </div>
