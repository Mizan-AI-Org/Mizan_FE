...
import { EmptyOpsState, AgentLoadingState, SectionHeader, SeverityBadge } from "@/components/os";
...
function StatusIcon({ item }: { item: AgentActivityItem }) {
...
type Props = {
  ...
  className?: string;
  compact?: boolean;
  /** When true, fetch from /api/agent/activity/ instead of using items prop */
  fetchRemote?: boolean;
  queryKey?: unknown[];
  title?: string;
  subtitle?: string;
  /** Skip outer section + header when embedded inside CommandCollapsibleSection */
  embedded?: boolean;
};
...
export function AgentActivityTimeline({
  ...
  const remote = useQuery({
    queryKey,
    queryFn: () => api.getAgentActivity({ limit: compact ? 12 : 40 }) as Promise<{ items: AgentActivityItem[] }>,
    enabled: fetchRemote,
    staleTime: 15_000,
  ...
  const detailQuery = useQuery({
    queryKey: ["agent", "activity", openId],
    queryFn: () => api.getAgentActivityDetail(openId!) as Promise<AgentActivityItem>,
    enabled: Boolean(openId),
  });

  const undo = useMutation({
    mutationFn: (id: string) => api.undoAgentActivity(id),
    onSuccess: () => {
      void qc.invalidateQueries({ queryKey: ["agent", "activity"] });
      void qc.invalidateQueries({ queryKey: ["agent", "command-center"] });
      if (openId) void qc.invalidateQueries({ queryKey: ["agent", "activity", openId] });
    },
  });
  ...
  const openEntity = (item: AgentActivityItem) => {
  ...
  const listBody = (
    <>
      {fetchRemote && remote.isLoading ? (
        <AgentLoadingState message={t("activity.loading")} variant="inline" />
      ) : items.length === 0 ? (
        <EmptyOpsState
        ...
        <ul
          className={cn(
            "divide-y divide-border/80 overflow-hidden",
            embedded ? "rounded-md border border-border/60 bg-muted/20" : "rounded-panel border border-border/80 bg-card",
          )}
        >
          {items.slice(0, compact ? 8 : 40).map((ev) => {
            const line = ev.action || ev.summary || t("activity.agent_action");
              ...
              >
                ...
                <div className="flex flex-wrap gap-1">
                  ...
                  <Button
                    ...
                    onClick={() =>
                      askAgent({
                        prompt: ev.ask_agent_prompt || t("activity.explain_prompt", { action: line }),
                        pageContext: {
    ...
    >
      ...
      <div className="relative z-10 max-h-[85vh] w-full max-w-lg overflow-y-auto rounded-panel border border-border bg-card p-5 shadow-strong">
        {detailQuery.isLoading && !openItem?.detail ? (
          <AgentLoadingState message={t("activity.loading_detail")} variant="inline" />
        ) : (
          <>
  ...
  return (
    <section aria-label="Agent activity" className={cn("space-y-3", className)}>
