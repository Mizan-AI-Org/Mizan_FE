...
const EnhancedScheduleView: React.FC = () => {
  ...
  const handleSaveShift = async (shift: Shift) => {
    ...
    try {
      ...
      const url = `${API_BASE}/scheduling/weekly-schedules/${scheduleData.id}/assigned-shifts/${isUpdate ? shift.id + "/" : ""}`;
  ...
  const handleDeleteShift = async (shiftId: string) => {
    ...
    try {
      const token = localStorage.getItem("access_token");
      const url = `${API_BASE}/scheduling/weekly-schedules/${scheduleData.id}/assigned-shifts/${shiftId}/`;
      const response = await fetch(url, {
        method: "DELETE",
        headers: { Authorization: `Bearer ${token}` },
      });
