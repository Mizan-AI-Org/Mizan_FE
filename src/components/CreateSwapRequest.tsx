...
const formSchema = z.object({
    ...
    receiver: z.string().uuid({ message: "Invalid staff member selected" }).optional().nullable(),
...
const CreateSwapRequest: React.FC<CreateSwapRequestProps> = ({ onSuccess }) => {
    ...
    const { data: myShifts, isLoading: isLoadingShifts } = useQuery<AssignedShift[]>({ // Specify the type of data expected
    ...
    const { data: allStaff, isLoading: isLoadingStaff } = useQuery<StaffMember[]>({ // Specify the type of data expected
        queryKey: ['allStaff', user?.restaurant?.id],
        queryFn: async () => {
    ...
    const createSwapRequestMutation = useMutation({
        mutationFn: async (data: CreateSwapRequestFormValues) => {
            const response = await fetch(`${API_BASE}/schedule/shift-swap-requests/`, {
                method: 'POST',
                headers: {
            ...
            <DialogContent className="sm:max-w-[425px]">
                ...
                <Form {...form}>
                    <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
                            ...
                            render={({ field }) => (
                                <FormItem>
                                    ...
                                    <Select onValueChange={field.onChange} defaultValue={field.value}>
                                        ...
                                        <SelectContent>
                                            {isLoadingShifts ? (
                                                <SelectItem value="" disabled>Loading shifts...</SelectItem>
                                            ) : myShifts?.length === 0 ? (
                                                <SelectItem value="" disabled>No upcoming shifts</SelectItem>
                                            ) : (
                                                myShifts?.map(shift => (
                                                    <SelectItem key={shift.id} value={shift.id}>
                                                        {format(new Date(shift.shift_date), 'PPP')} {format(new Date(`2000-01-01T${shift.start_time}`), 'hh:mm a')} - {format(new Date(`2000-01-01T${shift.end_time}`), 'hh:mm a')}
                                                        ({shift.role})
                                                    </SelectItem>
                                                ))
                                            )}
                            ...
                            render={({ field }) => (
                                <FormItem>
                                    ...
                                    <Select onValueChange={field.onChange} defaultValue={field.value || ''}>
                                        ...
                                        <SelectContent>
                                            <SelectItem value="">No specific receiver</SelectItem>
                                            {isLoadingStaff ? (
                                                <SelectItem value="" disabled>Loading staff...</SelectItem>
                                            ) : (
                                                allStaff?.filter(staff => staff.id !== user?.id).map(staff => (
                        ...
                        <FormField
                            ...
                            render={({ field }) => (
                                <FormItem>
                                    ...
                                    <FormControl>
                                        <Textarea placeholder="Reason for swap request" {...field} />
                                    </FormControl>
                                    <FormMessage />
                                </FormItem>
                            )}
                        ...
                        <Button type="submit" className="w-full" disabled={createSwapRequestMutation.isPending}>
